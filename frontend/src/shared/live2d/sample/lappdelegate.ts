...
export class LAppDelegate {
  ...
  /**
   * Сменить модель по индексу из ModelDir (0 … ModelDirSize-1).
   */
  public setModelSceneIndex(index: number): void {
    if (!this._subdelegates?.length) {
      return;
    }
    const safe =
      ((index % LAppDefine.ModelDirSize) + LAppDefine.ModelDirSize) %
      LAppDefine.ModelDirSize;
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager().addModel(safe);
    }
  }

  /**
   * Режим взгляда за указателем (из Live2DCanvas).
   * `_target` зарезервирован (canvas/document).
   */
  public setPointerFollow(
    mode: Live2DLookAtPointer,
    _target: Live2DFollowPointerTarget = "canvas",
  ): void {
    if (!this._subdelegates?.length) {
      return;
    }
    ...
    void _target;
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].setPointerFollow(mode);
    }
  }
   ...
   * APPに必要な物を初期化する。
   */
  public initialize(container: HTMLElement = document.body): boolean {
    // Cubism SDKの初期化
    this.initializeCubism();

    this.initializeSubdelegates(container);
   ...
   */
  private initializeSubdelegates(container: HTMLElement): void {
