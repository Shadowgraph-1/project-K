...
- [x] Тесты: критичные сервисы — см. [§20 Тестирование](#20-тестирование)
...
> Unit-тесты backend — в [§20 Тестирование](#20-тестирование).
...
В проекте есть **unit-тесты критичных сервисов backend** на [Vitest](https://vitest.dev/). Они проверяют логику авторизации, личного кабинета и валидацию входных данных. **PostgreSQL и запущенный API не нужны** — [Prisma](#glossary-prisma) в тестах замокан.

E2E-тесты UI (Playwright/Cypress) **не реализованы** — это указано в [ограничениях](#13-функциональные-требования-moscow).

### Что покрыто

| Область | Файл | Сценарии |
| ------- | ---- | -------- |
| Регистрация и вход | `backend/src/services/auth.service.test.ts` | Успешная регистрация, `email_taken`, закрытая регистрация (`forbidden`), вход, `invalid_credentials` |
| Личный кабинет | `backend/src/services/user.service.test.ts` | Обновление профиля, смена email, `email_taken`, смена пароля, `invalid_password`, удаление аккаунта |
| Валидация API | `backend/src/schemas/user.schema.test.ts` | Zod-схемы профиля, пароля и удаления аккаунта |

Конфигурация: `backend/vitest.config.ts`.
...
npm test       # один прогон, exit code 0 при успехе
```
...
Проверка типов backend (отдельно от тестов):
