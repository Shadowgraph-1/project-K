...
import { normalizeTaskPriority, TaskPriorityIcon, getTaskPriorityLabel } from "../shared/task-priority-icons";
import { TaskStatusIcon } from "../shared/task-status-icons";
import {
  TaskDateContextMenuFields,
  type TaskDateField,
} from "../shared/task-date-picker";
import {
  ContextMenuActionItem,
  linearContextMenuContentClass,
  TaskDeleteFieldIcon,
} from "../shared/task-context-menu";
...
import { TASK_ROW_GRID_COLUMNS } from "../../model/sessionWorkspaceTypes";
import { formatDate, formatShortDate } from "../../lib/format-activity-date";
...
} from "../shared/task-picker-menus";
import { SessionTooltip } from "../../../layout/SessionTooltip";
import { SubtaskProgressBadge } from "../shared/SubtaskProgressRing";
...
export function TaskRow({
  ...
  /** Blocks row open right after a picker/menu interaction (click-through). */
  const suppressOpenRef = useRef(false);
  const suppressOpenTimeoutRef = useRef<number | null>(null);

  const suppressRowOpen = useCallback(() => {
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <li
          ...
          onClick={handleOpen}
          onKeyDown={(event) =>
            handleRowOpenKeyDown(event, handleOpen)
          }
        >
        ...
        <div className="relative flex items-center justify-end gap-2">
          {subtaskProgress && subtaskProgress.total > 0 ? (
            <SessionTooltip
              label={`Подзадачи: ${subtaskProgress.done} из ${subtaskProgress.total}`}
            >
