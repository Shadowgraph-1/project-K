...
<img src="./frontend/docs/readme_logo.jpg" alt="Обложка проекта Focus With Me" width="420">
...
- [Focus With Me](#focus-with-me)
  ...
  - [Клиентские пути](#клиентские-пути)
  - [Функциональные требования MoSCoW](#функциональные-требования-moscow)
...
**Простыми словами.** Многим сложно долго держаться у экрана: таймеры холодные, награды абстрактные, чувство «я тут один» быстро утомляет и сбивает. **Focus With Me** добавляет рядом **персонажа-компаньона** — того, с кем «идёт» блок фокуса: он подбадривает, реагирует на результат сессии и в перспективе общается через ИИ в характере персонажа. Базовая механика остаётся знакомой: **фокус-интервалы в духе Pomodoro**, но опыт становится живее и связаннее с эмоцией пользователя.

**Какую боль закрывает продукт.** Обычный таймер не даёт ни привычки возвращаться, ни ощущения прогресса . В итоге чаще бросают задачу на полпути. Продукт поможет снизить этот порог за счёт **привязки к персонажу**, **понятной статистики**, **мотивационных сообщений** и в перспективе **напоминаний**, если человек выпал из ритма.

---

## 2. Целевая аудитория

На первом шаге продукт ориентируется на **студентов и фрилансеров**, которым нужно работать дома без жёсткого внешнего расписания. Им полезнее не просто считать минуты, а **держать ритм** без ощущения бесконечной работы: мягкая эмоциональная поддержка и геймификация здесь должны помогать, а не давить.
...
| Маршрутизация         | React Router                             | **Внедрено** (`/`, `/profile`, заглушка 404 и др.) |
| Стилизация            | Tailwind CSS v4 (`@tailwindcss/vite`)    | **Внедрено**                                       |
| Управление состоянием | Zustand                                  | **Частично** (например, модалки, auth store)       |
| API                   | Python, **FastAPI**, Pydantic            | Каркас backend; полноценное API — следующий шаг    |
...
Ниже — не формальный бизнес-процесс, а **цепочки «куда тыкает человек»**, чтобы было видно общий охват приложения до детальных user story.

**1. Первый контакт.** Пользователь открывает **главную (лендинг/онбординг)** — понимает идею, видит блоки про таймер и компаньонов. Из шапки или кнопки открывает **вход или регистрацию**.

**2. Фокус-сессия.** После авторизации человек попадает в сценарий **«выбрал компаньона → запустил таймер Pomodoro → ждёт / работает»**. По окончании или при досрочном выходе видит реакцию персонажа и (позже) начисление опыта или streak.

**3. Послеработка.** После нескольких сессий человек может зайти в **статистику и профиль**: посмотреть историю, streak, возможно изменить активного компаньона. Через **магазин** тратит внутреннюю валюту или открывает новых персонажей.

**4. Администратор.** Отдельный вход в **админ-часть**: правка каталога компаньонов, товаров, просмотр агрегированной активности пользователей (без «ручной правки каждого JSON» по SSH).

**Побочный путь.** Пользователь вводит битый URL — видит понятную **страницу 404**, откуда можно вернуться на главную.

---

## Функциональные требования MoSCoW

Я предпологаю такой маршрут, **что обязательно для первого рабочего контура**, а что можно отложить.

| Категория | Что входит (по смыслу) |
| --------- | ---------------------- |
| **Must** | Регистрация и вход, запуск и отображение таймера фокуса, привязка сессии к пользователю и компаньону (модель данных), базовая история сессий, API + БД для этого ядра, клиентские экраны главной и сессии в связке с API. |
| **Should** | Персонаж в UI с реакциями на завершение/срыв, статистика и streak, магазин и внутренняя валюта, напоминания по e-mail при долгом отсутствии. |
| **Could** | ИИ-диалог в стиле компаньона (LLM), Telegram-бот как второй канал, OAuth-вход, расширенные визуализации. |
| **Won’t (сейчас)** | Мобильное нативное приложение, офлайн-first без сервера, сложные корпоративные роли и интеграции с внешними таск-трекерами — вне скоупа текущей итерации, их не обещаем в срок. |

---

## 12. Пользовательские сценарии

Формулировки в формате **«Как …, я хочу …, чтобы …»** — так проще сверяться с требованиями и тестами.

1. Как **пользователь**, я хочу **зарегистрироваться и войти**, чтобы **мои сессии и прогресс сохранялись между визитами**.
2. Как **пользователь**, я хочу **запускать таймер фокуса с выбранным компаньоном**, чтобы **отделять работу на блоки и не чувствовать себя «одному в тишине»**.
3. Как **пользователь**, я хочу **видеть реакцию компаньона после сессии**, чтобы **получать эмоциональное подкрепление за завершённый блок**.
4. Как **пользователь**, я хочу **открывать историю и метрики (время, посещения и т.д.)**, чтобы **понимать свою динамику и не слетать с ритма**.
5. Как **пользователь**, я хочу **открывать магазин и тратить валюту на нового компаньона или оформление**, чтобы **было за что возвращаться в приложение**.
6. Как **пользователь**, я хочу **получать мягкое напоминание, если долго не захожу**, чтобы **не терять контекст незавершённых задач**.
7. Как **пользователь**, я хочу **общаться с компаньоном в стиле выбранного персонажа (ИИ)**, чтобы **поддержка ощущалась живой, а не шаблонной**.
8. Как **администратор**, я хочу **управлять каталогом компаньонов и товаров через интерфейс**, чтобы **не зависеть от разработчика для контентных правок**.
...
- [x] Перечень экранов
- [x] Клиентские пути (упрощённо, в README)
- [x] Приоритеты по MoSCoW (черновик, в README)
- [ ] Wireframes в Figma (главная, таймер, магазин) — **частично**, макеты будут меняться по мере вёрстки
...
- [x] React Router (базовые маршруты и 404)
- [x] Layout на ключевых страницах: шапка, навигация, подвал *(в объёме текущего UI)*
- [ ] Базовые UI-компоненты довести до единой дизайн-системы (Button, Input, Card, Modal)
- [ ] Zustand: полноценные `authStore`, `sessionStore` в связке с API
- [ ] Страницы входа и регистрации как отдельные маршруты *(сейчас часть сценария в модалке)*
...
- [x] README в корне репозитория с описанием продукта и запуском *(финальная полировка и демо-сценарий — к релизу)*
...
project-K/
├── frontend/
│   ├── docs/                # Материалы для README (например обложка)
│   ├── src/
...
| **`frontend/docs/`** | Вспомогательные материалы для документации (например изображение обложки в корневом README). |
